...
<!-- CSS Link  -->
<link href="resources/css/buyPage.css" rel="stylesheet">
<!--  jQuery 
    ...
    <div id="main">
       ...
       <hr>
        ...
        <div id="itemList" class="page_aticle order_goodslist"><div class="info_product"><a class="btn" href="#none"><span class="screen_out">펼침 / 닫힘</span></a>
          <div class="short_info"> dd외 <span class="num">3개</span>상품을 주문합니다.</div></div>
          <ul class="list_product">
          	...
          	<c:forEach var='i' items='${carts}' >
	            <li>
	              ...
	              <div class="thumb">
	              	<c:forEach var='a' items='${images}' varStatus='vs'>
          ...
          </ul><input type="hidden" id="isPickupType" value="false"></div>  <!-- 이건 뭐여.. -->       
  		
  		<!-- <form action="insertOrder.sp" method="post"> -->
        <!-- 주문 정보 전달 -->
        <!-- totalPrice, orderRequest, emailId, orderAmount, productNo -->
        <!-- 주소, 연락처(없으면 Member에 저장된 값 불러오기) -->   
		<table>
			<!-- 받는 사람 이름 -->
			<tr>
				하나 이상 미기재 시 고객 정보로 배송정보가 입력됩니다.
			</tr>
			<tr>
				<th>받는 분 성명 </th>
				<td><input type="text" name="recipientName" width="350px" value="${loginUser.userName }"></td>
			</tr>
			
			<!-- 연락처 -->
			<tr>
				<th>받는 분 연락처 </th>
				<td><input type="text" name="recipientPhone" value="" width="350px" value="${loginUser.phone }"></td>				
			</tr>
			...
			<tr>
				<th>우편번호</th>
				<td>
					<input type="text" name="post" class="postcodify_postcode5" value="" size="6">						<button type="button" id="postcodify_search_button">검색</button>
				</td>
			</tr>
			<tr>
				<th>도로명 주소</th>
				<td><input type="text" name="adr1" class="postcodify_address" value=""></td>
			</tr>
			<tr>		
				<th>상세 주소</th>
				<td><input type="text" name="adr2" class="postcodify_extra_info" value=""></td>
			</tr>
...
<!-- 			<tr>
				<th>배송지 </th>
				<td><input type="text" name="recipientAdr" width="350px"></td>				
			</tr> -->
			<!-- 요청 -->
			<tr>
				<th>요청 사항 </th>
				<td>
					<textarea name="orderRequest" value="" cols="50" row="10">
						ex.문앞에 놓고 가주세요
					</textarea>
				</td>
			</tr>
		...
		<script>
			$('#pay').on('click', function(){
				...
				$("input[name=cartNo]").each(function(index,item){
					cartNos.push($(item).val());
				});
				
				location.href = 'insertOrder.sp?cartNos=' + cartNos + '&recipientName='
						+ recipientName + '&recipientPhone=' + recipientPhone + 
						'&post=' + post + '&adr1=' + adr1 + '&adr2=' + adr2 
						+ '&totalPrice=' + totalPrice + '&orderRequest=' + orderRequest;
			});
